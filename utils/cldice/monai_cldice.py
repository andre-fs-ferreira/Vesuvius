...
class BCESoftDiceclDiceLoss(_Loss):
    """
    Compute the Soft clDice loss defined in:
    ...
    Includes the BCE and Soft dice 
    ...
    def __init__(self, iter_: int = 3, smooth: float = 1.0) -> None:
        ...
        self.iter = iter_
        self.smooth = smooth
        # alpha expected in the forward pass (not eficient but better to test)
        # self.alpha = alpha

    def forward(self, y_true: torch.Tensor, y_pred: torch.Tensor, roi_mask:torch.Tensor, weights) -> torch.Tensor:
        """
        y_true: Binary mask
        y_pred: Logits from the output model
        """
        y_true = y_true * roi_mask # gt masked
        
        # BCE
        bce_loss = binary_cross_entropy_with_logits(input=y_pred, target=y_true, reduction='none')
        bce_loss = bce_loss * roi_mask
        bce_loss = (bce_loss.sum() / (roi_mask.sum() + 1e-8))

        
        y_pred_prob = sigmoid(y_pred)*roi_mask # mask after activation

        # Soft dice
        dice = soft_dice(y_true, y_pred_prob, self.smooth)

        # Soft cldice
        skel_pred = soft_skel(y_pred_prob, self.iter)
        skel_true = soft_skel(y_true, self.iter)
        tprec = (torch.sum(torch.multiply(skel_pred, y_true)[:, :, ...]) + self.smooth) / (
            torch.sum(skel_pred[:, :, ...]) + self.smooth
        )
        ...
        total_loss: torch.Tensor =  weights[0]*bce_loss +  weights[1]*dice + weights[2]*cl_dice
        return total_loss